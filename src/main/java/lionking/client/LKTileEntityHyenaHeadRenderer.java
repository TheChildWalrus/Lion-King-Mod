...
public class LKTileEntityHyenaHeadRenderer extends TileEntitySpecialRenderer {
	...
	private final ModelBase model = new LKModelHyenaHead(false);
	...
	private void renderHead(float f, float f1, float f2, int metadata, float rotation, int hyenaType) {
		...
		if (hyenaType == 3) {
			...
			if (textures.get(hyenaType) == null) {
				textures.put(hyenaType, new ResourceLocation("lionking:mob/hyena_" + hyenaType + ".png"));
			}
			bindTexture((ResourceLocation) textures.get(hyenaType));
		}
