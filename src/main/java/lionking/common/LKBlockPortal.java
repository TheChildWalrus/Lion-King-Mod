...
public class LKBlockPortal extends BlockBreakable {
	...
	public static boolean tryToCreatePortal(World world, int i, int j, int k) {
		...
		int l = 0;
		int i1 = 0;
		if (world.getBlockId(i3 - 1, j, k2) == mod_LionKing.lionPortalFrame.blockID || world.getBlockId(i3 + 1, j, k2) == mod_LionKing.lionPortalFrame.blockID) {
			l = 1;
		}
		if (world.getBlockId(i3, j, k2 - 1) == mod_LionKing.lionPortalFrame.blockID || world.getBlockId(i3, j, k2 + 1) == mod_LionKing.lionPortalFrame.blockID) {
			i1 = 1;
		}
		if (l == i1) {
			return false;
		...
		for (int k1 = 0; k1 < 2; k1++) {
			for (int i2 = 0; i2 < 3; i2++) {
				world.setBlock(i3 + l * k1, j + i2, k2 + i1 * k1, mod_LionKing.lionPortal.blockID, 0, 2);
			}
